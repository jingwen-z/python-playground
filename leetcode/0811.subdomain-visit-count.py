...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        count_paired_dict = collections.Counter()
        for cpdomain in cpdomains:
            nb, domain = cpdomain.split(' ')
            nb = int(nb)
            subdomains = domain.split('.')
            for i in range(len(subdomains)):
                joined_domain = '.'.join(subdomains[i:])
                count_paired_dict[joined_domain] += nb
        out_list = [str(v) + ' ' + k for k, v in count_paired_dict.items()]
        return out_list
...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for cpdomain in cpdomains:
            nb, domain = cpdomain.split(' ')
            nb = int(nb)
            if not domain in count_paired_dict:
                count_paired_dict[domain] = nb
            else:
                count_paired_dict[domain] += nb

            subdomains = domain.split('.')
            for i in np.arange(-1, len(subdomains)*-1, -1):
                if i == -1:
                    if not subdomains[i] in count_paired_dict:
                        count_paired_dict[subdomains[i]] = nb
                    else:
                        count_paired_dict[subdomains[i]] += nb
                elif i < -1:
                    joined_domain = '.'.join(subdomains[i:])
                    if not joined_domain in count_paired_dict:
                        count_paired_dict[joined_domain] = nb
                    else:
                        count_paired_dict[joined_domain] += nb
        out_list = [str(v) + ' ' + k for k, v in count_paired_dict.items()]
