    ...
    def numUniqueEmails(self, emails: List[str]) -> int:
        ...
        for email in emails:
            ...
            if '.' in local and '+' in local:
                b4plus, afterplus = local.split('+', 1)
                b4plus_no_periods = b4plus.replace('.', '')
                combination = b4plus_no_periods + '@' + domain
                email_list.append(combination)
            elif '.' in local and not '+' in local:
                local_no_periods = local.replace('.', '')
                combination = local_no_periods + '@' + domain
                email_list.append(combination)
            elif '+' in local and not '.' in local:
                b4plus, afterplus = local.split('+', 1)
                combination = b4plus + '@' + domain
                email_list.append(combination)
            else:
                email_list.append(email)

        unique_nb = len(set(email_list))
        return unique_nb
